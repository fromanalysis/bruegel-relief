...
**→ [Read it](https://fromanalysis.github.io/bruegel-relief/)** · **→ [Make one in your browser](https://fromanalysis.github.io/bruegel-relief/painting2lamp.html)**
...
The one exception is `painting2lamp.html`, which is the app rather than a page: it is
JavaScript throughout — a port of the lamp path, with Pillow's LANCZOS reimplemented to the
letter so the resize is bit-exact — and it still ships as a single file that runs offline from
a `file://` URL. It is the only thing here that needs a browser to do more than render text.
